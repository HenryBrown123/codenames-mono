...
export const ARToggleButton = styled.button<{ $arMode: boolean }>`
  ...
  bottom: 2rem;
  right: 2rem;
  background: ${props => props.$arMode 
    ? 'rgba(0, 0, 0, 0.8)' 
    : 'white'};
  border: 2px solid ${props => props.$arMode 
    ? '#00ff88' 
    : '#333'};
  color: ${props => props.$arMode 
    ? '#00ff88' 
    : '#333'};
  ...
  font-size: 0.9rem;
  box-shadow: ${props => props.$arMode 
    ? '0 0 20px rgba(0, 255, 136, 0.3)'
    : '0 4px 12px rgba(0, 0, 0, 0.1)'};
  font-family: ${props => props.$arMode 
    ? "'JetBrains Mono', monospace" 
    : 'inherit'};
  text-transform: ${props => props.$arMode ? 'uppercase' : 'none'};
  letter-spacing: ${props => props.$arMode ? '0.05em' : 'normal'};
